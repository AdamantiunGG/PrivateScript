		...
		local Note = {
			...
			Update = function(self, num, listLength, dt)
				...
				if self.enabled then
					...
					time += estep * dt * 128 -- TODO duration
					estep += eestep * dt * 64
				end
			end,
...
MultiThreadList({
	function()
		AT_IMAGES[1] = game:HttpGet("https://i.imgur.com/9NMuFcQ.png")
	end,
	function()
		AT_IMAGES[2] = game:HttpGet("https://i.imgur.com/jG3NjxN.png")
	end,
	function()
		AT_IMAGES[3] = game:HttpGet("https://i.imgur.com/2Ty4u2O.png")
	end,
	function()
		AT_IMAGES[4] = game:HttpGet("https://i.imgur.com/kNGuTlj.png")
	end,
	function()
		AT_IMAGES[5] = game:HttpGet("https://i.imgur.com/OZUR3EY.png")
	end,
	function()
		AT_IMAGES[6] = game:HttpGet("https://i.imgur.com/3HGuyVa.png")
	end,
})
...
do
	...
	function Draw:ImageWithText(size, image, text, x, y, tab)
		...
		Draw:MenuImage(true, AT_IMAGES[5], x + 2, y + 2, size, size, 1, tab)
		table.insert(temptable, tab[#tab])
...
function menu.Initialize(menutable)
	...
	do
		...
		Draw:MenuBigText(MenuName or "aitech", true, false, 6, 6, bbmenu)
	...
	ColorpickerImage(false, AT_IMAGES[1], 12, 25, 156, 156, 1, cp.drawings)
	...
	table.insert(alphabar, cp.drawings[#cp.drawings])
	ColorpickerImage(false, AT_IMAGES[2], 12, 191, 159, 10, 1, cp.drawings)
	table.insert(alphabar, cp.drawings[#cp.drawings])
	...
	ColorpickerImage(false, AT_IMAGES[3], 178, 25, 10, 156, 1, cp.drawings)
	...
	ColorpickerImage(false, AT_IMAGES[4], 199, 39, 71, 36, 1, cp.drawings)
	...
	ColorpickerImage(false, AT_IMAGES[4], 199, 93, 71, 36, 1, cp.drawings)
	...
	local function buttonpressed(bp)
		...
		FireEvent("bb_buttonpressed", bp.tab, bp.groupbox, bp.name) --yes i know this is very inefficient and may cause crashes, but i don't care to re-write it bc it works
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			delfile("aitech/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".at")
			CreateNotification('Deleted "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.at"!')
			UpdateConfigs()
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					local menutext = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") or "aitech"
...
if menu.game == "Phantom Forces" then --SECTION PF BEGIN
	...
	for i = 1, 20 do
		...
		Draw:Image(false, AT_IMAGES[6], 20, 20, 23, 30, 1, nade_esp[4])
	...
	menu.pfunload = function(self)
		...
		for k, v in next, getinstances() do -- hacky way of getting rid of aitech adornments and such, but oh well lol it works
	...
	local chatspams = {
		...
		[4] = {
			...
			"tech 有的东西给了他，所以他不那rblx trader captain么有的东西给了他爱欠绝参加锻squidward炼，务，却拒绝参加锻炼，这让他爱得更UGT少了",
			"wocky slush他爱欠债了他他squilliam拥有的东西给爱欠绝参加锻squidward炼",
			"坐下，一直保持着安静的状态aitech 谁把他拥有的东西给了他，所以他不那rblx trader captain么有的东西给了他爱欠债了他他squilliam拥有的东西给爱欠绝参加锻squidward炼，务，却拒绝参加锻炼，这让他爱得更UGT少了",
			"免费手榴弹tech hack绕过作弊工作Phantom Force roblox aimbot瞄准无声目标绕过2020工作真正免费下载和使用",
		...
		[6] = {
			...
			"how does it feel to not have aitech 🤣🤣🤣😂😂😹😹😹 ",
			"im a firing my laza! 🙀🙀🙀 ",
			"😂😂😂😂😂GAMING CHAIR😂😂😂😂😂",
			"retardheadass",
			...
			'table.find(charts, "any other script other than aitech") -> nil 💵💵💵',
			...
			"y",
			"b",
			"before aitech 😭 📢				after aitech 😁😁😜					don't be like the person who doesn't have aitech",
			"							MADE YOU LOOK ",
			...
			"god i wish i had aitech..... 🙏🙏🥺🥺🥺													plzzzzz brooooo 🛐 GIVE IT🛐🛐",
			"buh bot 												",
			"votekick him!!!!!!! 😠 vk VK VK VK VOTEKICK HIM!!!!!!!!! 😠 😢 VOTE KICK !!!!! PRESS Y WHY DIDNT U PRESS Y LOL!!!!!! 😭 ", -- shufy made this
			"aitech omg omggg omggg its aitech its aitech OMGGG!!!  🙏🙏🥺🥺😌😒😡",
			"HOw do you get ACCESS to this AITECH ", -- end
			"I NEED ACCESS 🔑🔓 TO AITECH 🤖📃📃📃 👈 THIS THING CALLED AITECH SCRIPT, I NEED IT ",
	...
	local spam_words = {
		...
		"No Virus All Clean", "No Survey", "No Ads", "Free", "Not Paid", "Real", "REAL 2020", "2020", "Real 2017", "AITECH", "Cracked", "bbot CRACKED by QUARANTINE", "2014", "desuhook crack",
		"Aimware", "Hacks", "Cheats", "Exploits", "(FREE)", "🕶😎", "😎", "😂", "😛", "paste bin", "aitech script", "hard code", "正免费下载和使", "SERVER BACKDOOR",
		...
		"Owns All", "Trolling", "Troll", "Grief", "Kill", "弗吉艾尺艾杰开", "AITECH Developers", "Logic", "And", "and", "Glitch",
			...
			if client.char.alive then
				if self == workspace.Camera then
					if id == "CFrame" then
						...
						-- this is old an outdated code that doesnt fucking work!!! why!!!! TODO FIX NOW PLZ THX!
	...
	do --ANCHOR camera function definitions.
		function camera:AngleToEdge(angle, inset) -- pasted from the grenade indicator lmao
			local pos
		...
		function camera:IsVisible(Part, origin) origin = origin or Camera.CFrame.Position
			...
			-- return (position == Part.Position or (Parent and hit and Parent:IsAncestorOf(hit))) --this causes a memory leak and an instant crash. todo fix 
		end
	...
	do --ANCHOR ragebot definitions
		...
		function ragebot:GetTarget(hitboxPriority, players, origin)
							...
							if bone.ClassName == "Part" then
								...
								if fovToBone < aimbotFov or aimbotFov > 180 then -- Awesome
									...
									elseif autowall then
										------debug.profilebegin("AT self Penetration Check " .. player.Name)
										...
										--debug.profileend("AT self Penetration Check " .. player.Name)
									end
								end
							end
						end
			...
			--debug.profileend("AT self GetTarget")

			return cpart, theplayer, closest, firepos, head
		end

		function ragebot:GetKnifeTargets(flight) --this code doesnt fucking work because of an anticheat update pushed by stylis. fuck u stylis!!!!!!
		...
		if AT.username == "aitech dev" then
		...
		function ragebot:HitscanOnAxes(origin, person, bodypart, hitboxshift)
			...
			local dest = typeof(bodypart) ~= "Vector3" and bodypart.Position or bodypart -- fuck stylis
    ...
    for hash, func in next, clienteventfuncs do
		...
		if found6 then
			clienteventfuncs[hash] = function(killer, victim, dist, weapon, head) --this chunk of code is responsible for 
				if killer == LOCAL_PLAYER and victim ~= LOCAL_PLAYER then
					if menu:GetVal("Misc", "Sounds", "Kill Sound") then
						...
						soundid = soundEmpty and "rbxassetid://0" or soundid
				...
				if victim ~= LOCAL_PLAYER then
					if victim == ragebot.firsttarget then
						ragebot.firsttarget = nil
					end --this shit doesnt work! wwhy
	...
	do
					...
					for i = 1, #vm do
						...
						if model.Name:match(".*Arm$") then
							...
							for j = 1, #children do
								...
								if part.ClassName == "MeshPart" or part.Name == "Sleeve" then --this shit doesnt work anymore lol 
						...
						if not model.Name:match(".*Arm$") then
							...
							for j = 1, #children do
								...
								if part.ClassName == "MeshPart" or part.Name == "Sleeve" then
									--part.Color = menu:GetVal("Visuals", "Local", "Weapon Chams", COLOR1, true) --neither does this shit
									if part.Transparency ~= 1 then
										part.Transparency = 0
									end
								end
		...
		do --ANCHOR send hook
			...
			do -- ANCHOR Legitbot definition defines legit functions (yes it is very inefficient but idgaf because its retarded and works)
			...
			menu.Initialize({
				...
				{
					name = "Settings", --this shit does work
					content = {
						{
							name = "Player List",
							x = menu.columns.left,
			...
			do
				...
				local function updateplist()
					...
					for i, team in pairs(TEAMS:GetTeams()) do
						...
						table.sort(sorted_players) --TODO fix this shit, it doesn't work and i dont know why!!
					...
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						if menu.tabnames[menu.activetab] == "Settings" and menu.open then
							...
							if selectedPlayer ~= nil then
								if menu:MouseInMenu(28, 68, 448, 238) then --TODO this causes a memory leak and is very inefficient fix asap :))
...
do
	...
	wm.textString = " | " .. AT.username .. " | " .. os.date("%b. %d, %Y")
...
CreateNotification(string.format("finished loading the phantom forces cheat. (%d ms)", menu.load_time))
CreateNotification("press right shift to open and close the menu!")
CreateThread(function()
